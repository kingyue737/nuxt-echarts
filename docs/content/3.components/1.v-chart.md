...
:::note
Chart events can be bound in lower-case (`@datazoom`), kebab-case (`@brush-end`) or camelCase (`@dataZoom`) — all resolve to the same ECharts event. ZRender events use the `zr:` prefix, e.g. `@zr:mouse-move` or `@zr:mouseMove`.
:::
...
- `getVisual`
- [`renderToCanvas`](https://echarts.apache.org/en/api.html#renderToCanvas)
- [`renderToSVGString`](https://echarts.apache.org/en/api.html#renderToSVGString)
- `getSvgDataURL`
