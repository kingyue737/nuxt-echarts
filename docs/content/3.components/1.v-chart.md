...
::field-group
  ...
  :::field{name="loading-type" type="string"}
  Type of the loading effect shown while `loading` is `true`. Defaults to `default`; custom effects can be added with [`echarts.registerLoading`](https://echarts.apache.org/en/api.html#echarts.registerLoading) and selected by name. Effect-specific fields go in `loading-options`.
  :::
...
:::note
In templates, event names are camelized by Vue's compiler: `@brush-end` becomes `onBrushEnd` and `@zr:mouse-move` becomes `onZr:mouseMove`, so kebab-case and camelCase both work. ECharts itself only normalizes case, so `@datazoom` and `@dataZoom` register the same handler.

A kebab-case key written outside a template (a render function or JSX, e.g. `onBrush-end`) is **not** camelized by Vue, and ECharts would register the literal `brush-end` that never fires — use `onBrushEnd` there.

ZRender events use the `zr:` prefix, e.g. `@zr:mouseMove`.
:::
...
The `#graphic` slot lets you build the ECharts [`graphic`](https://echarts.apache.org/en/option.html#graphic) tree with Vue components instead of the `option.graphic` array. It replaces `option.graphic`; supplying both logs a warning. The `option` prop may be omitted for graphic-only charts.

All graphic element components — `GGroup`, `GRect`, `GCircle`, `GEllipse`, `GText`, `GImage`, `GLine`, `GPolygon`, `GPolyline`, `GArc`, `GBezierCurve`, `GRing`, `GSector` — are auto-imported from `vue-echarts/graphic`, no manual import needed.
...
:::note
Graphic slots only work on client-rendered components (`<VChart>`, `<VChartFull>`). Server-side rendering components (`<VChartServer>`, `<VChartLight>`, `<VChartIsland>`) render `option.graphic` from the static option only.
:::
